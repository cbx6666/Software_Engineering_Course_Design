...
- result_vis.png   ：给人看的“合成图”（把关灯图与纯投影反射叠加，便于直观看差异）
...
def find_input_images(data_dir: str) -> Tuple[str, str]:
    """
    在 data 目录里自动找到关灯图和开灯图。
    
    扫描 data 目录，找到两张输入图片。优先匹配文件名中包含 "env"（关灯图）和 "mix"（开灯图）的图片。
    如果找不到，就按文件名排序，取前两张。用户可能给图片起不同的名字，这个函数能自动识别，不需要手动指定路径。
    如果文件名符合规范（包含 env/mix），就能准确识别；否则用排序作为后备方案。
    
    参数：
        data_dir: data 目录的路径（例如 "data"）
    
    返回：
        (env_path, mix_path) 两个文件路径的元组：
        - env_path: 关灯图的完整路径
        - mix_path: 开灯图的完整路径
    """
    # 支持的图片格式
    ...
    # 优先匹配：查找文件名中包含 "env" 和 "mix" 的图片
    env = None
    mix = None
    for f in files:
        name = os.path.basename(f).lower()
        if "env" in name and env is None:
            env = f
        elif "mix" in name and mix is None:
            mix = f
    ...
    raise FileNotFoundError("未找到成对图片（env/mix 或至少两张）")
...
def main():
    ...
    原理：
        ...
        10. 保存结果：
            - result_detect.png：用于角点检测的灰度图（最可靠）
            - result_vis.png：可视化合成图（关灯图 + 纯投影反射）
    ...
    输出说明：
        ...
        - result_vis.png：这是给人看的，把“纯投影反射”叠加到关灯图上，
                         可以直观地看到投影图案在玻璃上的效果。
    ...
    # 保存可视化合成图：关灯图与纯投影反射叠加，便于直观查看
    i_clean = np.clip(img_env_aligned.astype(np.float32) + r_proj_bgr.astype(np.float32), 0, 255).astype(np.uint8)
    vis_out = os.path.join(out_dir, "result_vis.png")
    safe_imwrite(vis_out, i_clean)
