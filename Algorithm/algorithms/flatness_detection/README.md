...
写入 `pointcloud_data.json` 前会同步过滤非有限数值：只有 `projected_points` 的三维坐标和对应 `projected_dists` 都为有限数时才导出，避免无效视差产生的 `NaN` 写入前后端传输 JSON。
...
对有效稀疏三维点执行鲁棒平面拟合，核心实现在：

```text
pointcloud_gen/utils/plane_fit.py
```

当前算法为 `SVD/PCA 总最小二乘 + MAD 鲁棒重拟合 + 可选高度带重拟合`。
...
内部同时保留 `z = ax + by + c` 形式，供距离计算和旧接口兼容。
...
距离单位为米，保存指标时转换为毫米。法向量会统一为 `normal[2] >= 0`，因此正距离表示点相对拟合平面更高。

拟合流程：

1. 过滤无效三维点，仅使用 `NaN/Inf` 之外的有效稀疏点。
2. 计算点云质心，将点云中心化。
3. 对中心化点云做 SVD 分解，取最小奇异值对应方向作为平面法向量。
4. 计算所有有效点到初始平面的正交残差。
5. 使用 MAD 估计鲁棒阈值，剔除明显离群点。
6. 若内点数量不少于 3，则用内点再次 SVD 拟合，得到 `svd_mad_refit` 平面。
7. 对最终残差分布做高度带判定；当低侧残差扩散明显大于高侧残差时，选择稳定高度带再次拟合，得到 `svd_mad_height_band_refit`。

术语说明：

- SVD：奇异值分解，用于从三维点云中找到最小方差方向；该方向就是总最小二乘意义下的平面法向量。
- PCA：主成分分析，本实现与 SVD 等价地使用点云方差方向；前两个主方向张成平面，最小方差方向为法线。
- 总最小二乘：最小化点到平面的正交距离，而不是只最小化 `Z` 方向误差，适合倾斜平面。
- MAD：中位数绝对偏差，用残差相对中位数的偏离估计离群点阈值，比均值和标准差更不容易被极端点拉偏。

高度带判定：

- 根据最终残差的 `p5 / p50 / p95` 判断低侧是否存在明显下沉趋势。
- 若低侧扩散显著大于高侧扩散，则选取较稳定的上侧高度带参与最终重拟合。
- 输出 `fit_mask`、`fit_height_range` 和 `height_range_applied` 供算法内部和可视化使用。
- 高度带不会改变前后端点云传输字段，仍只导出 `projected_points` 和 `projected_dists`。
...
- 3D 图中点颜色默认按原始 `Z` 值着色。
- 投影图和等高线按 `projected_dists`，即点到最终拟合平面的有符号高度着色。
- 若触发高度带或存在拟合高度范围，图中会以灰色半透明平面标出高度带上下边界。
- 超出拟合高度带的点会灰色虚化，等高线插值优先使用参与拟合高度带的点。
...
注意：`fit_mask` 只决定最终平面拟合时采用哪些点，以及可视化时哪些点被标记为拟合高度带内点；平整度指标仍统计所有有效稀疏点到最终拟合平面的距离，避免隐藏真实局部下沉或凸起。
